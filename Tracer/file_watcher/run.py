from get_processes import ProcessItr
...
time.sleep(3)
dtrace_pid = int(run_cmd('ps -e | awk \'$6 == "file_watcher/filetracer.dtrace" { print $1}\'')[0])
...
def to_be_ignored(line, watched_processes):
    if line['pid'] in {dtrace_pid, tracer_pid}:
        return True
    path = line.get("path", "")
    file_not_torta_backup = path.split('/')[-1].startswith('filetrace.txt') or backupfile_regex.match(path)
    pid_not_in_watched_processes = line["pid"] not in watched_processes.keys()
...
def handle_line(raw_line):
    try:
        ...
        watched_processes = next(process_itr)
        ...
        line['app'] = watched_processes[line.get('pid', '')]
